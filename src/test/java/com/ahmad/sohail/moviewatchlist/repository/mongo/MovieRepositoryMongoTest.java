...
public class MovieRepositoryMongoTest {
	...
	public void testFindByIdFound() {
		addTestMovieToDatabase("1", "Inception", 2010, "1", false);
		addTestMovieToDatabase("2", "The Matrix", 1999, "1", false);
		assertThat(movieRepository.findById("2")).isEqualTo(new Movie("2", "The Matrix", 1999, "1"));
	}
	...
	public void testSave() {
		...
		assertThat(readAllMoviesFromDatabase()).containsExactly(new Movie("1", "Inception", 2010, "1"));
	}
	...
	private void addTestMovieToDatabase(String id, String title, int year, String genreId, boolean watched) {
		movieCollection.insertOne(new Document().append("id", id).append("title", title).append("year", year)
				.append("genreId", genreId).append("watched", watched));
	}

	private List<Movie> readAllMoviesFromDatabase() {
		return StreamSupport.stream(movieCollection.find().spliterator(), false)
